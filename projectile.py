...
def read_projectiles_csv(file_path):
    ...
    columns_to_extract = [
        "Speed","Damage", "CrownTowerDamagePercent", "Pushback", "Radius", "AoeToAir", 
                ...
                for col in columns_to_extract:
                    ...
                    # Handle integer fields, setting missing values to -1
                        ...
                        if col in ["Damage", "CrownTowerDamagePercent", "Pushback", "Radius", 
                                "ProjectileRange", "MinDistance","Speed"]:  # These are integers
...
class Projectile():
    ...
    def update(self,arena):
        ...
        if hasattr(self.target,"summon_num")==False:
            attach_target=self.is_ellipse_touching_line(self.type,self.target.posX-self.target.W//2,self.target.posY,self.target.W)            
        else:
            #單體攻擊
            if self.AoeToGround==False and self.AoeToAir==False:
                ellipse_self = {"cx": self.posX, "cy": self.posY, "a": 1, "b": 1} 
            else:
                ellipse_self = {"cx": self.posX, "cy": self.posY, "a": self.width, "b": self.height} 
            ellipse_other = {"cx": self.target.posX, "cy": self.target.posY, "a": self.target.CollisionRadius/1000*GRID_WIDTH, "b": self.target.CollisionRadius/1000*GRID_HEIGHT} 
    ...
    def draw(self,screen):
        ...
        pygame.draw.ellipse(screen, ORANGE, (top_left_x, top_left_y, self.width, self.height))  
