...
class Building:
    ...
    def act(self, enemy_troops, arena):
        ...
        if self.current_time < self.DeployTime:
            ...
            if self.SpawnNumber:  # 生產建築
                if (
                    self.current_time - self.last_spawn_time > self.SpawnPauseTime
                    or self.last_spawn_time == 0
                ):
                    self.msg = "Spawning"
                    ...
                    enemy_main_tower = (
                        arena.enemy_castle
                        if self.type == "player"
                        else arena.player_castle
                    )
                    for i in range(self.SpawnNumber):
                        c_i = Character(
                            self.SpawnCharacter + "s",
                            arena,
                            enemy_left_tower,
                            enemy_right_tower,
                            enemy_main_tower,
                        ...
                        (
                            arena.player_queue.append(c_i)
                            if self.type == "player"
                            else arena.enemy_queue.append(c_i)
                        )
    ...
    def draw(self, screen, show_act=False):
        ...
        pygame.draw.rect(
            screen,
            BLUE if self.type == "blue" else RED,
            pygame.Rect(
                self.posX - 0.5 * self.W, self.posY - 0.5 * self.H, self.W, self.H
            ),
        )
        self.draw_elispse(
            screen,
