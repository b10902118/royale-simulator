...
class Spell():
    ...
    def are_ellipses_intersecting(self,ellipse_other,sample=360):
        ...
        # 橢圓2參數
        cx2, cy2, a2, b2 = ellipse_other["cx"], ellipse_other["cy"], ellipse_other["a"], ellipse_other["b"]
...
class Arrow(Spell):
    def __init__(self,targetX=0,targetY=0,type="player"):
        ...
        self.deck_img=pygame.image.load(f"card_img//{self.name}.webp") if os.path.exists(f"card_img//{self.name}.webp") else None
        ...
        self.arrow_set=[[self.posX,self.posY,self.origin_radius,self.origin_radius] for _ in range(self.attack_num)]
        self.has_attacked=[False for _ in range(self.attack_num)]

        self.total_dist=((self.targetX-self.originX)**2+(self.targetY-self.originY)**2)**0.5
    ...
    def act(self,arena):
        ...
        for i in range(self.end_cnt,self.hit_cnt):
            ...
            if dist_now<5 and self.has_attacked[i]==False:# close to target position
                ...
                if self.type=="player":
                    enemy_l_tower,enemy_r_tower,enemy_main_tower=arena.enemy_left_tower,arena.enemy_right_tower,arena.enemy_castle
                else:
                    enemy_l_tower,enemy_r_tower,enemy_main_tower=arena.player_left_tower,arena.player_right_tower,arena.player_castle
                for t in [enemy_l_tower,enemy_r_tower,enemy_main_tower]:
                    ...
                    for x, y in rect_points:
                        if ((x - px) ** 2 / self.radiusX ** 2 + (y - py) ** 2 / self.radiusY ** 2) <= 1:
                    ...
                    if (dx ** 2 / self.radiusX ** 2 + dy ** 2 / self.radiusY ** 2) <= 1 and self.has_attacked[i]==False:
                        t.life-=self.tower_damage
                        self.has_attacked[i]=True
                ...
                self.arrow_set[i][0]+=dx * (self.speed / 100 * GRID_WIDTH) * DELTA_TIME
                self.arrow_set[i][1]+=dy * (self.speed / 100 * GRID_HEIGHT) * DELTA_TIME
    ...
    def hit(self,enemies):
        for e in enemies:
            if hasattr(e,"summon_num"):
                ellipse_enemy = {"cx": e.posX, "cy": e.posY, "a": e.CollisionRadius/1000*GRID_WIDTH, "b": e.CollisionRadius/1000*GRID_HEIGHT}
            else:
                ellipse_enemy = {"cx": e.posX, "cy": e.posY, "a": 0.5*e.W, "b": 0.5*e.H} 
    ...
    def draw(self,screen):
        for i in range(self.end_cnt,self.hit_cnt):
            px,py,rx,ry=self.arrow_set[i]
            text_surface = self.font.render(self.name if self.attack_num==1 else self.name+"_"+str(i+1), True, (255,255,255))
            text_rect = text_surface.get_rect(center=(px,py))
            screen.blit(text_surface, text_rect)
            pygame.draw.ellipse(screen, PURPLE,(px-rx ,py-ry ,2*rx, 2*ry),width=2)
...
class Fireball(Spell):
    ...
    def act(self,arena):
        ...
        if dist_now<5 and self.has_attacked==False:# close to target position
            ...
            for t in [enemy_l_tower,enemy_r_tower,enemy_main_tower]:
                ...
                if (dx ** 2 / self.radiusX ** 2 + dy ** 2 / self.radiusY ** 2) <= 1 and self.has_attacked==False:
                    t.life-=self.tower_damage
                    self.has_attacked=True
    ...
    def draw(self,screen):
        ...
        screen.blit(text_surface, text_rect)
        pygame.draw.ellipse(screen, PURPLE,(self.posX-self.radiusX ,self.posY-self.radiusY ,2*self.radiusX, 2*self.radiusY),width=2)
...
class Rage(Spell):
    ...
    def act(self,arena):
        ...
        if self.timer-self.last_buff_time>self.buff_interval:
            ...
            self_queue=arena.enemy_queue if self.type!="player" else arena.player_queue
            for c in self_queue:
                if hasattr(c,"summon_num"):
                    ellipse_enemy = {"cx": c.posX, "cy": c.posY, "a": c.CollisionRadius/1000*GRID_WIDTH, "b": c.CollisionRadius/1000*GRID_HEIGHT}
                    if self.are_ellipses_intersecting(ellipse_enemy):
                        c.rage_buff()
                            
                else:
                    ellipse_enemy = {"cx": c.posX, "cy": c.posY, "a": 0.5*c.W, "b": 0.5*c.H} 
                    if self.are_ellipses_intersecting(ellipse_enemy):
                        c.rage_buff()
                        
            if self.type!="player":
                l_tower,r_tower,main_tower=arena.enemy_left_tower,arena.enemy_right_tower,arena.enemy_castle
            else:
                l_tower,r_tower,main_tower=arena.player_left_tower,arena.player_right_tower,arena.player_castle
            for t in [l_tower,r_tower,main_tower]:
                ...
                for x, y in rect_points:
                    if ((x - self.targetX) ** 2 / self.radiusX ** 2 + (y - self.targetY) ** 2 / self.radiusY ** 2) <= 1:
                        t.rage_buff()
                        break
    ...
    def draw(self,screen):
        ...
        screen.blit(text_surface, text_rect)
        pygame.draw.ellipse(screen, PURPLE,(self.targetX-self.radiusX ,self.targetY-self.radiusY ,2*self.radiusX, 2*self.radiusY),width=2)
    ...
    def act(self,arena):
        ...
        if self.timer-self.last_buff_time>self.buff_interval:
            ...
            for t in [l_tower,r_tower,main_tower]:
                ...
                if (dx ** 2 / self.radiusX ** 2 + dy ** 2 / self.radiusY ** 2) <= 1 and self.has_damage==False:
                    self.has_damage=True
                    t.life-=self.tower_damage
        else:
            self.has_damage=False    
