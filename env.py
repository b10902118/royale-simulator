...
class GameWindow:
    ...
    def draw_deck_ui(self, player: Player, enemy: Player):
        # （敵方卡牌欄和聖水槽）
        pygame.draw.rect(
            self.screen, self.BLACK, (0, 0, self.SCREEN_WIDTH, self.ui_height)
        )

        # 卡牌槽
        for i in range(4):
            card_rect = pygame.Rect(
                2.5 * GRID_WIDTH + i * (self.card_width + 10),
                2.2 * GRID_HEIGHT,
                self.card_width,
                self.card_height,
            )
            if enemy.deck[i].deck_img:
                img = pygame.transform.scale(
                    enemy.deck[i].deck_img, (self.card_width, self.card_height)
                )  # Scale image to fit the card slot
                img.convert()
                self.screen.blit(
                    img, card_rect.topleft
        ...
        if enemy.deck[5].deck_img:
            img = pygame.transform.scale(
                enemy.deck[5].deck_img, (0.5 * self.card_width, 0.5 * self.card_height)
            )  # Scale image to fit the card slot
            img.convert()
            self.screen.blit(
                img, next_card_rect.topleft
            )  # Place the image at the calculated position
        else:
            # If no image, just draw a placeholder (you can customize it)
        ...
        for i in range(10):
            ...
            if i < int(enemy.elixir):
                pygame.draw.rect(
                    ...
                    self.PINK,
                    (
                        1.2 * GRID_WIDTH + i * (self.elixir_width + gap),
                        GRID_HEIGHT,
                        self.elixir_width,
                        self.elixir_height,
                    ),
        ...
        pygame.draw.rect(
            self.screen,
            self.BLACK,
            (0, self.SCREEN_HEIGHT - self.ui_height, self.SCREEN_WIDTH, self.ui_height),
        )
        ...
        for i in range(4):
            card_rect = pygame.Rect(
                2.5 * GRID_WIDTH + i * (self.card_width + 10),
                self.SCREEN_HEIGHT - 5.5 * GRID_HEIGHT,
                self.card_width,
                self.card_height,
            )
            if player.deck[i].deck_img:
                img = pygame.transform.scale(
                    player.deck[i].deck_img, (self.card_width, self.card_height)
                )  # Scale image to fit the card slot
                img.convert()
                self.screen.blit(
                    img, card_rect.topleft
        ...
        if player.deck[5].deck_img:
            img = pygame.transform.scale(
                player.deck[5].deck_img, (0.5 * self.card_width, 0.5 * self.card_height)
            )  # Scale image to fit the card slot
            img.convert()
            self.screen.blit(
                img, next_card_rect.topleft
        ...
        for i in range(10):
            ...
            if i < int(player.elixir):
                pygame.draw.rect(
                    ...
                    (
                        1.2 * GRID_WIDTH + i * (self.elixir_width + gap),
                        self.SCREEN_HEIGHT - 1.5 * GRID_HEIGHT,
                        self.elixir_width,
                        self.elixir_height,
                    ),
                )
